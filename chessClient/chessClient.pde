...
void rHighlight() {
  ...
  for (int i = row1 - 1; i>=0; i--) {
    if (grid[i][col1] != ' ') {
      if (Character.isUpperCase(grid[i][col1])) {
  ...
  for (int i = col1 - 1; i>=0; i--) {
    if (grid[row1][i] != ' ') {
      if (Character.isUpperCase(grid[row1][i])) {
...
void bHighlight() {
  for (int i = 1; (row1 - i >= 0) && (col1 - i >=0); i++) {
    if (grid[row1 - i][col1 - i] != ' ') {
      if (Character.isUpperCase(grid[row1 - i][col1 - i])) {
...
void nHighlight() {
  if (row1 - 2 >=0 && col1 + 1 < 8) {
    if (grid[row1 - 2][col1 + 1] == ' ' || Character.isUpperCase(grid[row1 - 2][col1 + 1])) {
  ...
  if (row1 - 1 >=0 && col1 + 2 < 8) {
    if (grid[row1 - 1][col1 + 2] == ' ' || Character.isUpperCase(grid[row1 - 1][col1 + 2])) {
...
void kHighlight() {
  for (int i = row1 - 1; i <= row1 + 1; i++) {
    for (int j = col1 - 1; j <= col1 + 1; j++) {
      if (i >= 0 && i < 8 && j >=0 && j < 8) {
        if(Character.isUpperCase(grid[i][j]) || grid[i][j] == ' '){
...
void pHighlight(){
  ...
  if( row1 > 0 && col1 > 0 && Character.isUpperCase(grid[row1 - 1][col1 - 1])){
    rect((col1 - 1) * gridSize, (row1 - 1) * gridSize, gridSize, gridSize);
    valid[row1 - 1][col1 - 1] = true;
  }
  if(row1 > 0 && col1 < 7 && Character.isUpperCase(grid[row1 - 1][col1 + 1])){
    rect((col1 + 1) * gridSize, (row1 - 1) * gridSize, gridSize, gridSize);
    valid[row1 - 1][col1 + 1] = true;
  }
}
